...
# %%
df = pd.read_csv("../original_data/BCI.csv")
df = df[df.LOCATION == "USA"].iloc[:, [5,6]]
df.columns = ["Date", "BCI"]
df.Date = pd.to_datetime(df.iloc[:,0], format='%Y-%m')
df.BCI = df.BCI.astype(float)
df.filter(items=['BCI']).hist(column = 'BCI', bins = 50) # Histogram before log transformation
df.BCI = df.BCI.apply(lambda x: np.log(x))
df.filter(items=['BCI']).hist(column = 'BCI', bins = 50) # Histogram after log transformation
df = df.dropna()
